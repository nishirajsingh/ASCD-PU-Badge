...
export default function Page() {
  ...
  const proceedToLinkedIn = async () => {
    ...
    const cloudUrl = 'https://cloudclubpu.me';
  
    // 🖥️ DESKTOP: Open new tab → NO clipboard, NO share
    if (!isMobile) {
      try {
        const linkedInWebUrl = `https://www.linkedin.com/feed/?shareActive=true&text=${encodedCaption}`;
        window.open(linkedInWebUrl, '_blank', 'noopener'); // OPEN IN NEW TAB ✔
      } catch (e) {
        console.error('Desktop new tab navigation failed, fallback', e);
    ...
    // 📱 MOBILE (clipboard + share + deep links)
    try {
      await navigator.clipboard.writeText(caption);
      console.log('Caption copied to clipboard (mobile)');
    } catch (e) {
